...
// const btnstart = document.getElementById('btnRestart');
...
document.querySelector('#pause').addEventListener('change', (e) => {
        ...
        document.querySelectorAll('.word').forEach((bubble) => {
            if (e.target.checked || bubble.style.webkitAnimationPlayState == "running" || bubble.style.webkitAnimationPlayState == "") {
                bubble.style.webkitAnimationPlayState = "paused"; // assuming you want to toggle
                music.pause();
                // alert("dung");
                document.querySelector('.icon-pause').querySelector('i').classList = 'fas fa-play';
            } else {
                if (bubble.style.webkitAnimationPlayState == "paused" &&
                    music_on) { music.play(); }
                bubble.style.webkitAnimationPlayState = "running";
                f1();
                document.querySelector('.icon-pause').querySelector('i').classList = 'fas fa-pause';
    ...
    //music on game
document.querySelector('#music-setting').addEventListener('change', (e) => {
    if (!e.target.checked) {
        music.muted = true;
        document.querySelector('#volume').classList = 'fas fa-volume-mute'
    } else {
        music.play();
        music.muted = false;
        music_on = true;
        document.querySelector('#volume').classList = 'fas fa-volume-up'
